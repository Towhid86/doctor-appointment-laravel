...
class RoleController extends Controller
{
    ...
    {
        $roles = Role::with('users')->where('name', '!=', 'author')->withCount('users')->get();
        return view('author.roles.index', compact('roles'));
    }
    ...
    public function create()
    {
        ...
        return view('author.roles.create', compact('groups'));
    }
...
//        sendNotification($this->role->id, route('roles.index'), __('New role has been created.'));
        return response()->json([
            'message' => __('Role created successfully'),
            'redirect' => route('author.roles.index')
        ]);
    }

    public function edit(Role $role)
    {
        abort_if($role->name == 'author', 403, __("You are not allowed to mess with Owner"));
        ...
        return view('author.roles.edit', compact('role', 'groups'));
    }
